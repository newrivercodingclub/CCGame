...
function canvasInit(){
    ...
    for( var x = 0; x < gridCoordinates.length; x++ ){
        ctx.strokeRect(gridCoordinates[x].x, gridCoordinates[x].y, squareSize, squareSize);
    }
    //stop drawing
    ctx.closePath();
...
async function keyHandler(event){
    ...
    if(!player.isMoving){ //player can only move if not already moving
        switch (event.key) {
...
async function arrowDown(){
    ...
    if(player.position[1] < height - squareSize){
        ...
        for(var x = 0; x < squareSize; x++){
            ...
            tempY += 1;
            ctx.strokeRect(player.position[0], player.position[1], squareSize, squareSize);
